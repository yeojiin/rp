...
// 강원도 : 서울오른쪽상단겹치는부분에서 시작
				...
				$.ajax({
					...
					success:function(data){
						console.log(data);
							...
							myPie.config.data.datasets[0].data = new Array();
							myPie.config.data.labels = new Array();
						for(var i=0; i<data.length; i++){
														
							myPie.config.data.datasets[0].data[i] = data[i].count;
							myPie.config.data.labels[i] = data[i].comType;
						}
	...
	$("#chart-area").css({"height":"300px"});
	
	
		
	var ctx = document.getElementById('chart-area').getContext('2d');
	window.myPie = new Chart(ctx,viewPie());
	
	
	var ctx = document.getElementsByTagName("canvas")[1].getContext('2d');
	window.myLine = new Chart(ctx, MonthSetChart());
		
	$(".searchBtn").click(function(){

		var startDate = $(this).siblings('input')[0].value.split("-").join("");
		var endDate = $(this).siblings('input')[1].value.split("-").join("");
		var value = $(".searchOption").val();
		console.log(value);
		$.ajax({
			url:"../../companyDetailStats.ad",
			type:"get",
			...
			success:function(data){
				...
				myLine.data.labels = new Array();
				 for(var i = 0; i<data.length; i++){
					myLine.data.labels[i] = data[i].enrollDate.split(" ")[0];
